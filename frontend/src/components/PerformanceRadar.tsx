...
const PerformanceRadar: React.FC<PerformanceRadarProps> = ({
  ...
  const [playerPlayedCounts, setPlayerPlayedCounts] = useState<
    ...
    const loadPerformanceData = async () => {
      try {
        ...
        for (const player of playersData) {
          ...
          const variance =
            matchCount > 0
              ? goalDifferences.reduce(
                  (sum, gd) => sum + Math.pow(gd - meanGD, 2),
                  0,
                ) / matchCount
              : 0;
        ...
        const averageMetrics = performanceMetrics.map((metric) => {
          const playerValues = playerNames.map(
            (name) => metric[name] as number,
          );
          const average =
            playerValues.reduce((sum, val) => sum + val, 0) /
            playerValues.length;
          return {
            ...metric,
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              {(hoveredPlayer || effectiveSelectedPlayer) === "all" ? (
                ...
                <Radar
                  name={
                    (hoveredPlayer || effectiveSelectedPlayer)
                      .charAt(0)
                      .toUpperCase() +
                    (hoveredPlayer || effectiveSelectedPlayer).slice(1)
                  }
                  dataKey={hoveredPlayer || effectiveSelectedPlayer}
                  stroke={getPlayerColor(
                    hoveredPlayer || effectiveSelectedPlayer,
                  )}
                  fill={getPlayerColor(
                    hoveredPlayer || effectiveSelectedPlayer,
