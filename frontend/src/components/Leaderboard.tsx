...
const Leaderboard: React.FC = () => {
  ...
  const rankedPlayers = players.filter((p) => p.played >= MIN_RANKED_MATCHES);
  const provisionalPlayers = players.filter((p) => p.played < MIN_RANKED_MATCHES);
  ...
  const renderPlayerRow = (player: EnhancedPlayer, position: number, highlightTop: boolean) => {
    ...
    return (
      ...
      >
        ...
        <TableCell align="center">
          ...
          >
            {highlightTop && getPositionIcon(position)}
            {!isMobile && (
              <Typography
                variant="h6"
                sx={{
                  ml: highlightTop && getPositionIcon(position) ? 1 : 0,
                  fontWeight: highlightTop && position <= 3 ? 700 : 400,
          ...
          <Table size={isMobile ? "small" : "medium"}>
            ...
            <TableBody>
              {rankedPlayers.map((player, index) =>
                renderPlayerRow(player, index + 1, true)
              )}
            </TableBody>
          </Table>
        </TableContainer>
        ...
        {provisionalPlayers.length > 0 && (
          <Box mt={4}>
            <Box display="flex" alignItems="center" mb={2}>
              <Typography variant="h5" component="h3" sx={{ opacity: 0.8 }}>
              ...
              <Typography
                ...
                sx={{ ml: 2, opacity: 0.5 }}
              >
                Fewer than {MIN_RANKED_MATCHES} matches played
              </Typography>
            </Box>
            <TableContainer>
              <Table size={isMobile ? "small" : "medium"}>
                {renderTableHeader()}
                <TableBody>
                  {provisionalPlayers.map((player, index) =>
                    renderPlayerRow(player, index + 1, false)
                  )}
                </TableBody>
              </Table>
            </TableContainer>
